...
@objc open class PusherConnection: NSObject {
    ...
    open var reconnectAttemptsMax: Int? = 100
    open var reconnectAttempts: Int = 0
    open var maxReconnectGapInSeconds: Double? = 60
...
//    Now we use half of activityTimeoutInterval
...
//    open var pongResponseTimeoutInterval: TimeInterval = 30
    ...
    */
    public init(
        ...
        self.socket = socket
        self.activityTimeoutInterval = options.activityTimeout ?? 30
...
//        }
    ...
    */
    @objc private func sendPing() {
        let dataString = JSONStringify([Constants.JSONKeys.event: Constants.Events.Pusher.ping,
                                        Constants.JSONKeys.data: [:]])
        socket.ping(string: dataString)
    ...
    */
    private func setupPongResponseTimeoutTimer() {
        pongResponseTimeoutTimer = Timer.scheduledTimer(
            timeInterval: activityTimeoutInterval / 2,
            target: self,
    ...
    */
    private func handleSubscriptionSucceededEvent(event: PusherEvent) {
        ...
        guard event.data != nil else {
            ...
            self.delegate?.subscribedToChannelNoDataInPayload?(name: channelName)
            return
        }
...
//        Use activityTimeout from socket server, taking into mobile network delays
        
        if options.activityTimeout == nil,
           let activityTimeoutValueFromServer = connectionData["activity_timeout"] as? String,
           let activityTimeoutFromServer  = Double(activityTimeoutValueFromServer) {
            self.activityTimeoutInterval = activityTimeoutFromServer - 5
        }
