...
open class NWWebSocket: WebSocketConnection {
    ...
    open func ping(string: String) {
        ...
        send(data: "ping".using(.utf8), context: context)
        */
    }
...
//        когда долго получаем сообщение о завершении соединения - из-за этого cancel мы завершаем очередь раньше, чем получаем ответ от сервера по сокетам - тогда соединение остается.
